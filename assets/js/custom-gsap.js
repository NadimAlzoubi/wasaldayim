...
// =================================== Custom Split text Js Start =====================================
if ($(".splitTextStyleOne").length > 0) {
  let elements = gsap.utils.toArray(".splitTextStyleOne");

  elements.forEach((el) => {
    // حفظ النص الأصلي للـ SEO
    const originalText = el.innerHTML;

    // تقسيم النص إلى كلمات فقط
    const split_text = new SplitText(el, {
      type: "words", // فقط كلمات
      lineThreshold: 0.5,
    });

    // Timeline مع ScrollTrigger
    const tl2 = gsap.timeline({
      scrollTrigger: {
        trigger: el,
        start: "top 90%",
    ...
    // تحريك الكلمات واحدة تلو الأخرى
    tl2.from(split_text.words, {
      autoAlpha: 0,
      y: 20, // حركة أصغر لأنها كلمة كاملة
      duration: 0.5,
      stagger: 0.05,
      ease: "back.out(1.7)",
    });

    // حفظ النص الأصلي في data attribute للـ SEO إذا لزم الأمر
